...
const PAIRS = {
  mentee: [
    ...
    {key:"home",      label:"Home",          note:'<b>Ask a mentor comes first</b>, then progress, then what to explore.'},
    ...
    {key:"community", label:"Community",     note:'<b>Video previews up front</b>, real answers from mentors, across Featured, Latest and Following.'},
    {key:"player",    label:"Video playback",note:'<b>A cleaner video player</b>, with controls that stay easy to read over any video.'},
    {key:"opps",      label:"Opportunities", note:'<b>Recommended for you</b>, with images and filters that make the right opportunity easier to spot.'},
    {key:"profile",   label:"Profile",       note:'One page instead of a settings maze: <b>who you are, your pathway, and what you actually use.</b>'},
    {key:"pathways",  label:"Pathways",      note:'<b>Typical pay by stage and the roles behind it</b>, in place of one long paragraph and a grid of job titles.'},
  ],
  mentor: [
    ...
    {key:"answer",    label:"Guided answers",note:'<b>Angles, roles and skills worth covering</b>, offered before you record. The question arrives already loaded from the web.'},
  ],
};
const NEW = [
  {key:"conversations",   title:"Conversations",    status:"#ece6df"},
  {key:"pulse-ai",        title:"Pulse AI",         status:"#f6f2ff"},
  {key:"career-timeline", title:"Navigator",        status:"#ece6df"},
  {key:"dark-mode",       title:"Dark mode",        status:"#141020"},
];
...
function phone(role, p, side, i){
  ...
  // `from` lets a pair borrow another role's captures (the shared Welcome screen).
  // data-role carries the SOURCE role because the lightbox builds paths from it.
  const src = p.from || role;
  return `<figure class="shot shot--${isNew?'new':'cur'}">
    <span class="lab ${isNew?'new':'cur'}">${isNew?'REDESIGN':"TODAY’S APP"}</span>
    <button class="zoomwrap" data-role="${src}" data-key="${p.key}" data-side="${side}" data-label="${p.label}"
      ...
      <div class="frame frame--${isNew?'new':'cur'}">
        ...
        <div class="screen"><img src="${shot(src,p.key,side)}" alt="${p.label}, ${isNew?'redesign':"today’s app"}"
  ...
  return `<article class="ncard" data-key="${item.key}">
    <div class="shotwrap">
      <button class="zoomwrap" data-mode="single" data-key="${item.key}" data-label="${item.title}"
        ...
        <div class="device" style="--status:${item.status}">
          <span class="zbadge" aria-hidden="true">&#10530;</span>
          <div class="device-screen"><img src="https://tparis7.github.io/P3-Mobile-App-Redesign/assets/shots/new/${item.key}.webp" alt="${item.title}"
...
// The heading is a plain label now, so a capture that 404s just loses its card.
// Only an empty grid changes anything: the band hides rather than heading nothing.
function recount(){
